...
class Story(// The story wrapper (caller - implements logging and callbacks)
        private val wrapper: StoryWrapper?, fileName: String?) : VariableMap {
    ...
    fun loadData(sNode: JsonNode, provider: StoryWrapper) {
        for (node in sNode.withArray(StoryJson.CONTENT)) {
            val id = node.get("id").asText()
            val content = storyContent[id]
            if (content != null) {
                if (node.has("#n"))
                ...
                if (node.has("vars")) {
                    ...
                    for (v in node.withArray("vars")) {
                        val `val` = loadObject(v, provider)
                        if (`val` != null)
                ...
                wrapper!!.logException(InkParseException("Could not identify content ID " + id + " while loading data for " + fileNames[fileNames.size - 1]))
            }
        }
        container = storyContent[sNode.get("currentContainer").asText()] as Container
        contentIdx = sNode.get("currentCounter").asInt()
        for (cNode in sNode.withArray("currentChoices")) {
            val cont = storyContent[cNode.asText()]
            if (cont != null)
                choices.add(cont as Container?)
            else {
    ...
    */
    ...
    fun addInterrupt(interrupt: StoryInterrupt) {
        ...
        if (interrupt.isChoice) {
            val choice = Choice(0, interrupt.interrupt, null)
            choice.id = interrupt.id
    ...
    internal fun initialize() {
        ...
        if (container != null && container!!.type == ContentType.KNOT) {
            if (container!!.getContent(0).isStitch)
                container = container!!.getContent(0) as Container
        }
        contentIdx = -1
        running = true
        processing = true
    ...
    private operator fun hasNext(): Boolean {
        ...
        if (container == null)
            return false
        return contentIdx < container!!.contentSize
    }
    ...
    private fun incrementContent(content: Content?) {
        ...
        contentIdx++
        if (container != null && contentIdx >= container!!.contentSize) {
            if (choices.isEmpty() && (container!!.isChoice || container!!.isGather)) {
                var c: Container = container
                var p = c.parent
                while (p != null) {
                    var i = p.getContentIndex(c) + 1
                    while (i < p.contentSize) {
                        val n = p.getContent(i)
                        if (n.isGather) {
            ...
            if (container!!.isConditional) {
                ...
                contentIdx = if (container != null) container!!.getContentIndex(oldContainer) + 1 else 0
            }
        } else {
            val next = content
    ...
    private val content: Content?
        ...
        get() {
            ...
            if (contentIdx >= container!!.contentSize)
                return null
    ...
    private fun resolveContent(content: Content): String {
        if (content.type == ContentType.TEXT) {
            val ret = if (content.isDivert) resolveDivert(content) else StoryText.getText(content.text, content.count, this)
            content.increment()
            return ret
        }
        if (content.isChoice) {
    ...
    private fun getDivertTarget(content: Content): Container? {
        ...
        if (divertTo.type == ContentType.KNOT && divertTo.getContent(0).isStitch)
            divertTo = divertTo.getContent(0) as Container
    ...
    internal fun add(content: Content) {
        if (content.id != null) {
            ...
            throw InkParseException("No ID for content. This should not be possible.")
        }
        // Set starting knot
    ...
    override fun debugInfo(): String {
        ...
        if (container != null && contentIdx < container!!.contentSize) {
            val cnt = container!!.getContent(contentIdx)
