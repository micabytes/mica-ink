...
@JvmOverloads constructor(expression: String, defaultMathContext: MathContext = MathContext.DECIMAL32) {
    ...
    (/// This operators name (pattern).
            val oper: String, /// Operators precedence.
            val precedence: Int, /// Operator is left associative.
    ...
    */
        ...
        while (tokenizer.hasNext()) {
            ...
            if (isNumber(token)) {
                ...
                while (token2 != null &&
                        ...
                        && (o1.isLeftAssoc && o1.precedence <= operators.get(token2).precedence || o1.precedence < operators.get(token2).precedence)) {
                    outputQueue.add(stack.pop())
    ...
    fun eval(story: VariableMap): Any {
        ...
        for (token in getRPN(story)) {
            if (operators.containsKey(token)) {
                ...
                val `var` = token.substring(0, token.indexOf("."))
                val function = token.substring(token.indexOf(".") + 1)
                val `val` = story.getValue(`var`)
                if (`val` == null) {
                    stack.push("")
                } else {
                    val p = ArrayList<Any>()
                    ...
                    try {
                        ...
                        var fResult = m.invoke(`val`, *params)
                        if (fResult is Int)
                        ...
                        var errMsg = "Could not identify a method " + function + " on variable " + `var` + " (" + `var`.toString() + ", " + valClass.getName() + ") with the parameters:"
                        for (i in paramTypes.indices)
                            errMsg += " " + paramTypes[i].getName()
                        errMsg += ". " + story.debugInfo()
                        throw InkRunTimeException(errMsg, e)
                    } catch (e: InvocationTargetException) {
                        var errMsg = "Could not invoke a method " + function + " on variable " + `var` + " (" + `var`.toString() + ", " + valClass.getName() + ") with the parameters:"
                        for (i in paramTypes.indices)
                            errMsg += " " + paramTypes[i].getName()
                        errMsg += ". " + story.debugInfo()
                        throw InkRunTimeException(errMsg, e)
                    } catch (e: IllegalAccessException) {
                        var errMsg = "Could not access a method " + function + " on variable " + `var` + " (" + `var`.toString() + ", " + valClass.getName() + ") with the parameters:"
                        for (i in paramTypes.indices)
                            errMsg += " " + paramTypes[i].getName()
                        errMsg += ". " + story.debugInfo()
                        throw InkRunTimeException(errMsg, e)
    ...
    fun addOperator(operator: Operator): Operator {
        return operators.put(operator.oper, operator)
    }
